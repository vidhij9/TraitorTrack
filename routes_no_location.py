...
def create_user():
    ...
    try:
        ...
        user.verified = True  # Admin-created users are auto-verified
...
def demote_user(user_id):
    ...
    try:
        ...
        if user.id == current_user.id:
            return jsonify({'success': False, 'error': 'Cannot demote yourself'})
...
def delete_user(user_id):
    ...
    try:
        ...
        scan_count = Scan.query.filter(Scan.user_id == user.id).count()
        if scan_count > 0:
            return jsonify({'success': False, 'error': f'Cannot delete user with {scan_count} scans'})
...
def create_bill():
    ...
    if form.validate_on_submit():
        try:
            ...
            parent_bag_count = form.parent_bag_count.data or 1
...
@app.route('/bill/<int:bill_id>/finish', methods=['POST'])
@login_required
def finish_bill_scan(bill_id):
    ...
    try:
        ...
        flash('Bill scanning completed!', 'success')
...
@app.route('/bill/remove_bag', methods=['POST'])
@login_required
def remove_bag_from_bill():
    ...
    try:
        ...
        if not bill_id or not bag_id:
            flash('Missing bill or bag ID.', 'error')
            return redirect(url_for('bill_management'))
        
        # Find and delete the link
        bill_bag = BillBag.query.filter_by(bill_id=bill_id, bag_id=bag_id).first()
        if bill_bag:
            db.session.delete(bill_bag)
            db.session.commit()
            flash('Parent bag removed from bill.', 'success')
        else:
        ...
        app.logger.error(f'Remove bag error: {str(e)}')
...
@app.route('/bill/<int:bill_id>/finish')
@login_required
