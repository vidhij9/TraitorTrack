    ...
    try:
        ...
        if hasattr(current_user, 'role') and current_user.role == 'admin':
            user_stats = {
                ...
                'active_today': db.session.query(func.count(func.distinct(Scan.user_id))).filter(
        ...
        hourly_rate = Scan.query.filter(Scan.timestamp >= hour_ago).count()
        ...
        # Bill stats (for admin and biller)
        bill_stats = {}
        if hasattr(current_user, 'role') and current_user.role in ['admin', 'biller']:
            bill_stats = {
                'total': Bill.query.count(),
                'completed': Bill.query.filter_by(status='completed').count(),
                'in_progress': Bill.query.filter_by(status='in_progress').count(),
                'avg_bags': db.session.query(func.avg(Bill.parent_bag_count)).scalar() or 0
            }
