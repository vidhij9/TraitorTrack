...
# Fix AWS Application Deployment
echo "Fixing AWS deployment..."

# Create a deployment script that will be sent to AWS instance
cat > aws-setup.sh <<'SETUP'
#!/bin/bash
...
# Send the script to AWS instance
aws ssm send-command \
    --instance-ids i-0057a68f7062dd425 \
    --document-name "AWS-RunShellScript" \
    --parameters 'commands=["'"$(cat aws-setup.sh | base64 -w0)"'" | base64 -d | bash"]' \
    --region ap-south-1 \
    --query "Command.CommandId" \
    --output text

echo "Deployment script sent to AWS instance"