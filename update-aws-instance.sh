...
INSTANCE_ID="i-085d0045035cabab5"
REGION="us-east-1"

echo "🚀 Updating AWS instance $INSTANCE_ID with fixed TraceTrack code..."

# Create the update command script
cat > /tmp/aws_update_commands.sh << 'EOFSCRIPT'
#!/bin/bash
set -e

echo "🔧 Stopping existing TraceTrack service..."
sudo pkill -f gunicorn || echo "No existing gunicorn process found"

cd /app || { echo "App directory not found"; exit 1; }

echo "📝 Backing up existing files..."
sudo cp app.py app.py.backup.$(date +%s) || echo "No existing app.py"

echo "🛠️ Creating fixed TraceTrack application..."

# Create fixed app_clean.py
...
sudo pip3 install Flask Flask-SQLAlchemy Flask-Login gunicorn bcrypt python-dotenv

echo "🔥 Setting up environment..."
export DATABASE_URL="sqlite:///tracetrack.db"
export SESSION_SECRET="aws-tracetrack-fixed-2025"

echo "🔧 Starting fixed TraceTrack application..."
...
echo "🎉 TraceTrack update complete!"
echo "📊 Application is running with fixed code on port 5000"

EOFSCRIPT

# Execute the update using AWS Systems Manager
echo "📡 Executing update on AWS instance via SSM..."
aws ssm send-command \
    --region $REGION \
    --instance-ids $INSTANCE_ID \
    --document-name "AWS-RunShellScript" \
    --parameters 'commands=["bash -c \"$(cat /dev/stdin)\""]' \
    --query 'Command.CommandId' \
    --output text < /tmp/aws_update_commands.sh

echo "✅ Update command sent to AWS instance!"
echo "⏳ Please wait 30-60 seconds for the update to complete..."