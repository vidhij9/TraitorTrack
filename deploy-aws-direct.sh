...
# Deploy application directly to AWS
...
cat > aws-deploy/requirements.txt <<'REQ'
Flask==2.3.2
...
cat > aws-deploy/deploy.sh <<'DEPLOY'
#!/bin/bash
...
# Create tarball
tar -czf aws-deploy.tar.gz -C aws-deploy .

echo "Deployment package created: aws-deploy.tar.gz"
echo "To deploy to AWS, copy this file to the EC2 instance and run the deploy.sh script"